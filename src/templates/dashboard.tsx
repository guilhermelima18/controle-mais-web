...
export function DashboardTemplate({ user }: DashboardTemplateProps) {
  ...
  const { transactionsDashboard, onListTransactionsDashboard } =
    useTransactions();
  ...
  if (!transactionsDashboard) return;
      ...
      <div className="space-y-8">
          ...
          <div className="relative flex flex-col gap-6 md:flex-row md:items-end md:justify-between">
            <div>
              ...
              <p className="mt-3 font-display text-5xl font-semibold tracking-tight md:text-6xl">
                {formatCurrency(
                  Math.round(
                    Number(transactionsDashboard?.total) * 100,
                  ).toString(),
                )}
              </p>
            </div>
            <div className="grid grid-cols-2 gap-3">
              <StatCard
                label="Entradas"
                value={transactionsDashboard?.income}
                type="income"
              />
              <StatCard
                label="Saídas"
                value={transactionsDashboard?.expense}
                type="expense"
              />
            </div>
          </div>
        ...
        <div className="grid gap-6 lg:grid-cols-2">
          <CategoryPanel
            ...
            total={transactionsDashboard?.income}
            items={typeIncome}
            accent="income"
          />
          <CategoryPanel
            title="Saídas por categoria"
            total={transactionsDashboard?.expense}
            items={typeExpense}
            accent="expense"
          />
        </div>
